...
const string programInfo = "\033[3m\033[1m\033[33mArk\033[32mCulator\033[0m\033[0m v5.2 (11.07.25) | Copyright 2022-2025 AEYCEN / TRAYVED | MIT License";
...
void calculateCost(const Weapon weapon, const int amount)
{
    ...
    if (element > 0)
        cout << "Element: " << ceil(element * amount) << endl;
...
int main()
{
    ...
    while (true) {
        try {
            ...
            string ammoName;
            while (true) {
                cout << "Enter " << BOLD << "weapon" << THIN << " [" << YELLOW << "1" << RESET << "(C4)/" << YELLOW << "2" << RESET << "(Rocket Launcher)/" << YELLOW << "3" << RESET << "(Tek Rifle)/" << YELLOW << "4" << RESET << "(Cannon)/" << YELLOW << "5" << RESET << "(Grenade)/" << YELLOW << "6" << RESET << "(Catapult)] > ";
                cin >> weaponInput;

                if (weaponInput >= 1 && weaponInput <= 6) {
                    break;
                }
                cout << RED << "Invalid weapon type. Please enter 1, 2, 3, 4, 5 or 6." << RESET << endl;
                cin.clear();
